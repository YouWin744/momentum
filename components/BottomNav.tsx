...
const BottomNav: React.FC<BottomNavProps> = ({ activeView, onViewChange }) => {
  return (
    <nav className="fixed bottom-0 left-0 right-0 h-[88px] ios-blur-nav border-t border-slate-200/50 dark:border-white/5 px-4 pb-[env(safe-area-inset-bottom,20px)] flex items-center justify-around z-40">
      <button 
      ...
      <div className="fixed bottom-1.5 left-1/2 -translate-x-1/2 w-32 h-1.5 bg-slate-900/10 dark:bg-white/10 rounded-full z-50"></div>
    </nav>
  );
};
